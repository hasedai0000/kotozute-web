...
| ログイン | `/login` | |
| 新規登録 | `/register` | |
| 招待の受諾 | `/invitations/[token]` | 有効期限つき使い捨てトークン。未ログインからログイン／登録へ導線 |

### 認証必須エリア（Route Group `(app)`）

| 画面 | パス | 備考 |
|---|---|---|
| ダッシュボード | `/dashboard` | 進捗と各セクションへの入口 |
| ノート一覧 | `/notebook` | |
| セクション編集 | `/notebook/[section]` | section＝`basic` / `medical` / `money` / `digital` / `funeral` / `pet` / `other`。1 実装で 8 領域を賄う |
| 大切な人へ | `/messages` | 手紙 |
| 家族・共有管理 | `/family` | 招待発行、メンバー一覧、権限 |
| プレビュー／書き出し | `/preview` | PDF・印刷 |
| 設定 | `/settings` | 公開タイミング、待機日数、リマインド、退会 |

### 死後開示（v1・パスのみ先に確定）

| 画面 | パス | 備考 |
|---|---|---|
| 発動（家族） | `/family/disclosure` | 確認モーダル＋再認証を経由 |
| 撤回（本人） | `/disclosure/[requestId]` | 通知メールから直接到達できるよう独立パス |

**URL 設計の原則**：パスに `family_id` / `user_id` を含めない（ログイン中のユーザーから解決する）。例外は招待トークンのみで、有効期限つき・DB はハッシュ保存。
